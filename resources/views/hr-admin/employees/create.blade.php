...
@section('page-title', 'Add New Employee Profile')
...
<div class="max-w-4xl mx-auto">
    <form method="POST" action="{{ route('hr-admin.employees.store') }}" class="space-y-6">
        @csrf
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700/60 shadow-xs space-y-4">
            <div class="border-b border-slate-100 dark:border-slate-700/60 pb-3">
                ...
                <p class="text-xs text-slate-400">Basic contact and identity profile of the employee.</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" id="date_of_birth" name="date_of_birth" required value="{{ old('date_of_birth', '2000-01-01') }}"
                        class="mt-1.5 block w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700/60 shadow-xs space-y-4">
            <div class="border-b border-slate-100 dark:border-slate-700/60 pb-3">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">2. System Account & Login Credentials</h3>
                <p class="text-xs text-slate-400">Credentials for the employee self-service portal.</p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="username" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                        Portal Username <span class="text-slate-400 text-[10px] lowercase font-normal">(leave blank to auto-generate)</span>
                    </label>
                ...
                <div>
                    <label for="password" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                        Initial Password <span class="text-slate-400 text-[10px] lowercase font-normal">(leave blank to auto-generate secure password)</span>
                    </label>
        ...
        <!-- Section 3: Employment Details & Shift Assignment -->
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700/60 shadow-xs space-y-4">
            <div class="border-b border-slate-100 dark:border-slate-700/60 pb-3">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">3. Employment & Shift Assignment</h3>
                <p class="text-xs text-slate-400">Designation, department, and work schedule governance.</p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="employee_code" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                        Employee Code <span class="text-slate-400 text-[10px] lowercase font-normal">(default: {{ $suggestedCode }})</span>
                    </label>
                    <input type="text" id="employee_code" name="employee_code" value="{{ old('employee_code', $suggestedCode) }}"
                ...
                <div>
                    ...
                    @error('shift_id') <p class="mt-1 text-[11px] text-rose-500">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="department" name="department" required value="{{ old('department', 'Engineering') }}"
                        class="mt-1.5 block w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                ...
                <div>
                    <label for="designation" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                        Designation / Role Title <span class="text-rose-500">*</span>
                    </label>
                ...
                <div>
                    ...
                    <input type="date" id="joining_date" name="joining_date" required value="{{ old('joining_date', date('Y-m-d')) }}"
                        class="mt-1.5 block w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700/60 shadow-xs space-y-4">
            <div class="border-b border-slate-100 dark:border-slate-700/60 pb-3">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">4. Compensation & Salary Structure</h3>
                <p class="text-xs text-slate-400">Monthly gross salary used for payroll and loss of pay calculations (divisor: 30 days).</p>
            </div>
            ...
            <div>
                ...
                <div class="mt-1.5 relative rounded-xl shadow-xs">
                    ...
                    <input type="number" step="0.01" min="0" id="monthly_salary" name="monthly_salary" required value="{{ old('monthly_salary') }}"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700/60 shadow-xs space-y-4">
            <div class="border-b border-slate-100 dark:border-slate-700/60 pb-3">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">5. Bank & Statutory Information</h3>
                <p class="text-xs text-slate-400">Direct salary disbursement and tax identification records.</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="bank_name" name="bank_name" required value="{{ old('bank_name', 'HDFC Bank') }}"
                        class="mt-1.5 block w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    @error('bank_name') <p class="mt-1 text-[11px] text-rose-500">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="account_number" name="account_number" required value="{{ old('account_number') }}"
                        class="mt-1.5 block w-full font-mono rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    @error('account_number') <p class="mt-1 text-[11px] text-rose-500">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="ifsc_code" name="ifsc_code" required value="{{ old('ifsc_code') }}"
                    ...
                    @error('ifsc_code') <p class="mt-1 text-[11px] text-rose-500">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="pan_number" name="pan_number" required value="{{ old('pan_number') }}"
                        placeholder="ABCDE1234F"
                        class="mt-1.5 block w-full uppercase font-mono rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    @error('pan_number') <p class="mt-1 text-[11px] text-rose-500">{{ $message }}</p> @enderror
                </div>
            </div>
        </div>

        <!-- Section 6: Initial Leave Balances Allocation -->
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700/60 shadow-xs space-y-4">
            <div class="border-b border-slate-100 dark:border-slate-700/60 pb-3">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">6. Initial Annual Leave Allocation ({{ date('Y') }})</h3>
                <p class="text-xs text-slate-400">Allocate starting annual quota for active company leave policies.</p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                @foreach($leaveTypes as $lt)
                    <div>
                        <label for="leave_{{ $lt->id }}" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                            {{ $lt->name }} (Days)
                        </label>
                        <input type="number" step="0.5" min="0" max="365"
                            id="leave_{{ $lt->id }}"
                            name="leave_allocations[{{ $lt->id }}]"
                            value="{{ old("leave_allocations.{$lt->id}", $lt->annual_quota) }}"
                            class="mt-1.5 block w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    </div>
                @endforeach
            </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit"
                ...
                Save & Provision Employee Profile
            </button>
        </div>
    </form>
</div>
