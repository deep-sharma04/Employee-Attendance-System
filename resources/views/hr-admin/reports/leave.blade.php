...
<div class="container mx-auto px-4 py-6">
    <h1 class="text-2xl font-semibold text-gray-800 mb-6">Leave Reports</h1>

    <div class="bg-white p-4 rounded-lg shadow-md mb-6">
        <form method="GET" action="{{ route('hr-admin.reports.leave') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
            ...
            <div class="md:col-span-5 flex justify-end gap-3">
                ...
                <a href="{{ route('hr-admin.reports.leave.export', request()->all()) }}" class="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold px-4 py-2 rounded-xl shadow-xs transition-colors flex items-center gap-1.5">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $record->leaveType->name }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ \Carbon\Carbon::parse($record->start_date)->format('d M') }} - {{ \Carbon\Carbon::parse($record->end_date)->format('d M Y') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 capitalize">
                        @if($record->status == 'approved') <span class="text-green-600">Approved</span> @elseif($record->status == 'rejected') <span class="text-red-600">Rejected</span> @else <span class="text-yellow-600">Pending</span> @endif
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate">{{ $record->reason }}</td>
                </tr>
                @empty
                <tr><td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">No records found.</td></tr>
                @endforelse
            </tbody>
        </table>
