...
@section('title', 'Create Shift Schedule')
@section('page-title', 'Create New Work Shift')
...
<div class="max-w-2xl mx-auto">
    <form method="POST" action="{{ route('hr-admin.shifts.store') }}" class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700/60 shadow-xs space-y-5">
        @csrf
        ...
        <div class="border-b border-slate-100 dark:border-slate-700/60 pb-3">
            ...
            <p class="text-xs text-slate-400">Configure core working hours and attendance grace parameters.</p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" id="code" name="code" required value="{{ old('code') }}"
                    placeholder="e.g. GEN_DAY or MORNING_01"
            ...
            <div>
                ...
                <input type="time" id="start_time" name="start_time" required value="{{ old('start_time', '09:00') }}"
                    class="mt-1.5 block w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
            ...
            <div>
                ...
                <input type="time" id="end_time" name="end_time" required value="{{ old('end_time', '18:00') }}"
                    class="mt-1.5 block w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
            ...
            <div>
                ...
                <input type="number" min="0" max="120" id="grace_period_minutes" name="grace_period_minutes" required value="{{ old('grace_period_minutes', 15) }}"
                    class="mt-1.5 block w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                <p class="mt-1 text-[11px] text-slate-400">Punches within grace period count as On-Time (Present).</p>
            ...
            <div>
                ...
                <input type="number" min="15" max="360" id="half_day_threshold_minutes" name="half_day_threshold_minutes" required value="{{ old('half_day_threshold_minutes', 60) }}"
                    class="mt-1.5 block w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                <p class="mt-1 text-[11px] text-slate-400">Late punches exceeding this duration convert to Half Day.</p>
                ...
                @foreach($allDays as $day)
                    <label class="flex items-center gap-2 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/30 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-all text-xs text-slate-800 dark:text-slate-200">
                        <input type="checkbox" name="working_days[]" value="{{ $day }}"
                            {{ in_array($day, old('working_days', ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'])) ? 'checked' : '' }}
        ...
        <div class="pt-2">
            <label class="flex items-center gap-2 cursor-pointer text-xs text-slate-700 dark:text-slate-300 font-semibold">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }}
                    class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                <span>Enable shift schedule immediately</span>
            </label>
        </div>
